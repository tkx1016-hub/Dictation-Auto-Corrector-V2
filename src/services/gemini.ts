    ...
    const prompt = `
      ...
      2. 针对每一个听写项，分别从以下共三项内容进行核对批改：
         ...
         - 词性 (Part of Speech): 词性缩写（如 n., v., adj., adv. 等）是否准确。
           - 【防止自动送分规则】：如果标准答案中包含了词性（如 n., v., adj. 等），但学生在作业图片中完全没有写词性，或者漏写了词性缩写，你必须判定其词性为错误 (posCorrect: false)，实际写的词性 (posActual) 记录为 "未写" 或 ""。绝不能因为学生没写词性就自动判对送分！
           - 【特殊情况】：仅当标准答案本身不含任何词性信息时（例如标准答案本身就写为 "dictionary 字典" ），词性才默认判定为正确 (posCorrect: true)，此时 posExpected 和 posActual 可以都设定为 "" 或 "N/A"。
